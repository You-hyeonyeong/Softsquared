...
router.post('/', async (req, res) => {
    ...
    if (!id || !pw || !name) {
        ...
        const selectUserResult = await db.queryParam_Parse(selectUser,[id]);
        if (selectUserResult.length >= 1) {
            res.status(200).send(utils.successFalse("이미 존재하는 아이디 입니다"));
        } else {
            //가입시키기
            const insertUserResult = await db.queryParam_Parse(insertUser, [id, pw, name]);
            const selectUserIdx = await db.queryParam_None('SELECT userIdx FROM market.user ORDER BY userIdx DESC LIMIT 1;')
